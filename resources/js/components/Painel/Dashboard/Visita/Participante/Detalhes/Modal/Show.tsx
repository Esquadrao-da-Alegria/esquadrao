            ...
            <DialogContent className="max-h-[90vh] overflow-y-auto border-amber-100 bg-white sm:max-w-2xl">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                    <Detalhe
                        ...
                        valor={String(participante.visitas_validas)}
                        detalhe={
                            participante.meta_mensal === null
                                ? 'Sem meta de visitas'
                                : `Meta de ${participante.meta_mensal} por mês`
                        }
                    />
                    <Detalhe
                        titulo="Saldo atual"
                        valor={
                            participante.saldo_atual === null
                                ? 'Não aplicável'
                                : `${participante.saldo_atual > 0 ? '+' : ''}${participante.saldo_atual}`
                        }
                        detalhe={
                            participante.compensacao_atual ??
                            'Sem compensação pendente'
                        }
                    />
                    <Detalhe
                        titulo="Reuniões"
                    ...
                    <Detalhe
                        titulo="Relatórios"
                        valor={`${participante.relatorios_pendentes} pendentes`}
                        detalhe={`${participante.relatorios_fora_prazo} fora do prazo`}
                    />
                    <Detalhe
                        titulo="Última atividade"
                        valor={
                            participante.ultima_atividade
                                ? new Date(
                                      participante.ultima_atividade,
                        ...
                        detalhe={
                            participante.dias_sem_atividade === null
                                ? 'Sem dados suficientes'
                                : `${participante.dias_sem_atividade} dias sem atividade documentada`
                        }
                    />
                </div>

                <p className="rounded-xl bg-amber-50 px-4 py-3 text-xs leading-relaxed text-amber-900/70">
                    Os dados apoiam a análise da coordenação e não geram
                    decisões automáticas sobre o voluntário.
                </p>
                ...
                <DialogFooter>
                    ...
                    <Link
                        href={participanteShow(participante.id, {
                            query: filtrosAtivos,
                        })}
                        className="inline-flex items-center justify-center rounded-full border-2 border-amber-600 bg-white px-5 py-2.5 font-semibold text-amber-700 transition hover:bg-amber-50"
                    >
