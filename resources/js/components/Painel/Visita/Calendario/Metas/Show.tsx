...
export interface AcompanhamentoMeta {
    ...
    planejadas_periodo: number | null;
    faltam_periodo: number;
    meta_mensal: number;
...
const Show: FC<Props> = ({ metas }) => {
    ...
    return (
        <section className="mt-4 overflow-hidden rounded-xl border border-amber-100 bg-white" aria-labelledby="titulo-acompanhamento-metas">
            ...
            <p className="border-b border-amber-50 px-3 py-1.5 text-[10px] text-amber-900/55">
                S: semana · Q: quinzena · M: mês. Valores mostram visitas cobertas / meta.
            </p>
            <div className="divide-y divide-amber-50 text-[11px] sm:text-xs">
                {metas.map((meta) => (
                    <div key={`${meta.hospital_id}-${meta.ala_id ?? 'geral'}`} className="flex min-w-0 items-center gap-2 px-3 py-2 text-amber-950">
                        ...
                        <span className="shrink-0 rounded-full bg-amber-50 px-2 py-0.5 tabular-nums" title={`${meta.titulo_periodo}: faltam ${meta.faltam_periodo} visitas`}>
                            {meta.sigla_periodo} {meta.meta_periodo === null ? '—' : `${meta.planejadas_periodo}/${meta.meta_periodo}`}
                        </span>
