...
interface Props {
    ...
    ajustes_contabilizacao?: {
        ...
        relatorios_atrasados: any[];
    };
}

const labelTipoParticipacao = (
    tipo: VisitaParticipante['tipo_participacao'],
) => (tipo === 'palhaco' ? 'Palhaço' : 'Paisana');

const labelStatusParticipacao = (
    status: VisitaParticipante['status_participacao'],
) => {
...
const Edit: FC<Props> = ({
    ...
    const [participacoes, setParticipacoes] = useState<VisitaParticipante[]>(
    ...
    const participantes = useMemo(
        () =>
            participacoes
                ...
                .sort((a, b) =>
                    (a.voluntario?.name ?? '').localeCompare(
                        b.voluntario?.name ?? '',
                    ),
                ),
        [participacoes],
    ...
    const handleAdicionarParticipante = async () => {
        ...
        try {
            const response = await fetch(
                `/visitas/${visita.id}/participantes`,
                {
                    method: 'POST',
                    headers: {
            ...
            if (response.ok && res.sucesso) {
                ...
                toast.error(
                    res.erros?.[0] ?? 'Erro ao adicionar participante.',
                );
            }
        } catch {
    ...
    const handleRemoverParticipante = async (
        ...
        if (
            !window.confirm(
                `Deseja remover ${participante.voluntario?.name ?? 'este participante'} da visita?`,
            )
        ) {
            return;
        }
        ...
        try {
            const response = await fetch(
                `/visitas/${visita.id}/participantes/${participante.id}`,
                {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleSubmit = () => {
        if (novoVoluntarioId) {
            toast.warning(
                'Clique em “Adicionar” para incluir o participante selecionado antes de salvar os dados gerais da visita.',
            );
            return;
        }
                        ...
                        <div className="overflow-hidden rounded-3xl border bg-white">
                            <div className="p-8 md:p-12">
                                    ...
                                    <div>
                                                    ...
                                                    >
                                                        <div>
                                                            ...
                                                            <span className="ml-2 text-xs text-gray-500">
                                                                (
                                                                {labelTipoParticipacao(
                                                                    p.tipo_participacao,
                                                                )}
                                                                {' · '}
                                                                {labelStatusParticipacao(
                                        ...
                                        <div className="rounded-2xl border border-gray-200 bg-gray-50/60 p-4">
                                            ...
                                            <p className="mb-3 text-xs text-gray-500">
                                                Selecione a pessoa e clique em{' '}
                                                <strong>Adicionar</strong>. A
                                                inclusão é feita imediatamente e
                                                não depende do botão Salvar.
                                            </p>
                                ...
                                {eh_administrador && ajustes_contabilizacao && (
                                    <div className="mt-6">
                                        <AjusteContabilizacao
                                            visitaId={visita.id!}
                                            voluntarios={
                                                ajustes_contabilizacao.voluntarios
                                            }
                                            relatorios={
                                                ajustes_contabilizacao.relatorios_atrasados
                                            }
                                            ajustes={
                                                ajustes_contabilizacao.ajustes
                                            }
                                        />
                                    </div>
                                )}
