...
const Campo: FC<{ label: string; valor?: string | null }> = ({
    label,
    valor,
}) => {
    if (!valor) {
    ...
    return (
        <div>
            ...
            <dd className="mt-1 text-sm whitespace-pre-wrap text-amber-950">
                {valor}
            </dd>
        </div>
    );
                ...
                <div className="overflow-hidden rounded-3xl border border-amber-100 bg-white shadow-sm">
                    <div className="border-b border-amber-50 px-6 py-8 sm:px-8">
                        ...
                        {relatorio.fora_do_prazo && (
                            <p className="mt-4 inline-flex rounded-full border border-orange-200 bg-orange-50 px-3 py-1 text-xs font-medium tracking-wide text-orange-800 uppercase">
                        ...
                        <dl className="space-y-5">
                            {relatorio.alaUnidade ? (
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm text-amber-950">
                                        {relatorio.alaUnidade.nome}
                                    </dd>
                                </div>
                            ) : null}
                            <Campo
                                label="Unidades / Alas visitadas"
                                valor={relatorio.unidades_visitadas}
                            />
                            <Campo label="Resumo" valor={relatorio.resumo} />
                            <Campo
                                label="Feedback"
                                valor={relatorio.feedback}
                            />
                            ...
                            {relatorio.pessoas_impactadas != null && (
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm text-amber-950">
                                        {relatorio.pessoas_impactadas}
                                    </dd>
                                </div>
                            )}
                            <Campo
                                label="Observação sobre visitantes externos"
                                valor={relatorio.observacao_visitantes_externos}
                            />
                            <Campo
                                label="Observações gerais"
                                valor={relatorio.observacoes_gerais}
                            />
                        </dl>
                    </div>
                </div>
