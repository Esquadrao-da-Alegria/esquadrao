    ...
    const { data, setData, transform, put, processing, errors } =
        useForm<EventoFormValues>({
            ...
            data_fim: evento.data_fim
                ? extrairData(evento.data_fim)
                : extrairData(evento.data_inicio),
            sem_hora_fim:
                !evento.data_fim || extrairHora(evento.data_fim) === '23:59',
            hora_fim:
                evento.data_fim && extrairHora(evento.data_fim) !== '23:59'
                    ? extrairHora(evento.data_fim)
                    : '',
            limite_inscricao_data: evento.limite_inscricao
                ? extrairData(evento.limite_inscricao)
                : '',
            limite_inscricao_hora: evento.limite_inscricao
                ? extrairHora(evento.limite_inscricao)
                : '',
