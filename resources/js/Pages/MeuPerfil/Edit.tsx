...
import { type Cargo, type SharedData, type Voluntario } from '@/types'
...
const Edit: FC<Props> = ({ voluntario }) => {
    ...
    const [fotoPreview, setFotoPreview] = useState<string | null>(voluntario?.url_foto ?? null)
        ...
        return (
            <PainelLayout>
                ...
                <section className="mx-auto max-w-3xl px-4 py-16">
                    <p className="text-gray-600">Perfil de voluntário não encontrado para este usuário.</p>
                </section>
            </PainelLayout>
        )
    }

    const cargos: Cargo[] = (auth.user.cargos as Cargo[]) ?? []

    return (
        <PainelLayout>
            <Head title="Meu perfil" />
            <section className="mx-auto w-full max-w-3xl px-4 py-16">
                <h1 className="mb-8 text-3xl font-bold text-amber-800">Meu perfil</h1>

                {/* Foto de perfil */}
                <div className="mb-6 rounded-3xl border bg-white p-8">
                    <h2 className="mb-4 text-lg font-semibold text-gray-800">Foto de perfil</h2>
                    <div className="flex flex-wrap items-center gap-6">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            >
                                ...
                                {uploadando ? 'Enviando...' : fotoPreview ? 'Alterar foto' : 'Adicionar foto'}
                            </button>
                            {fotoPreview && (
                                <button
                                    type="button"
                            ...
                            <p className="text-xs text-gray-500">JPG, PNG ou WebP. Máximo 2 MB.</p>
                        </div>
                        <input
                            ref={fileInputRef}
                            type="file"
                ...
                {/* Dados pessoais */}
                <form onSubmit={handleSubmit} className="mb-6 rounded-3xl border bg-white p-8">
                    <h2 className="mb-4 text-lg font-semibold text-gray-800">Dados pessoais</h2>
                    <div className="space-y-4">
                        <div>
                            <label htmlFor="nome_completo" className={painelLabelClass}>
                                Nome completo *
                            </label>
                            <input
                                id="nome_completo"
                        ...
                        <div>
                            <label htmlFor="nome_doutor" className={painelLabelClass}>
                                Nome doutor
                            </label>
                            <input
                                id="nome_doutor"
                                value={data.nome_doutor}
                                onChange={(e) => setData('nome_doutor', e.target.value)}
                                className={painelInputClass}
                                placeholder="Ex: Dr. Alegria"
                            />
                            {errors.nome_doutor && (
                                <p className="mt-1 text-sm text-red-600">{errors.nome_doutor}</p>
                            )}
                        </div>

                        <div>
                            <label htmlFor="telefone" className={painelLabelClass}>
                                Telefone
                            </label>
                            <input
                                id="telefone"
                        ...
                        <div>
                            <label htmlFor="data_nascimento" className={painelLabelClass}>
                                Data de nascimento
                            </label>
                            <input
                                type="date"
                ...
                <div className="rounded-3xl border bg-white p-8">
                    <h2 className="mb-4 text-lg font-semibold text-gray-800">Informações da ONG</h2>
                    <dl className="space-y-3 text-sm">
                        <div>
                            <dt className="font-medium text-gray-500">Cidade base</dt>
                        ...
                        <div>
                            ...
                            <dd className="mt-0.5 text-gray-900">
                                {LABEL_STATUS[voluntario.status] ?? voluntario.status}
                            </dd>
                        </div>
                        {voluntario.data_entrada_ong && (
                            <div>
                                ...
                                <dd className="mt-0.5 text-gray-900">
                                    {new Date(voluntario.data_entrada_ong + 'T00:00:00').toLocaleDateString('pt-BR')}
                                </dd>
                            </div>
                        )}
                        {cargos.length > 0 && (
                            <div>
                                <dt className="font-medium text-gray-500">Cargos</dt>
                                <dd className="mt-0.5 text-gray-900">
