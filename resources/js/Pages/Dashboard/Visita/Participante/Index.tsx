    ...
    return (
        <PainelLayout>
            ...
            <div className="mx-auto max-w-7xl space-y-6 px-5 py-8 sm:px-6 lg:px-8">
                <header>
                    <p className="text-sm font-semibold text-amber-700">Apoio à decisão</p>
                    <h1 className="mt-1 text-2xl font-semibold text-amber-950 sm:text-3xl">Participação dos voluntários</h1>
                ...
                <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Card icon={CheckCircle2} titulo="Dentro da meta" valor={indicadores.dentro_meta} tom="verde" />
                    <Card icon={AlertTriangle} titulo="Requer análise" valor={indicadores.requer_analise} tom="vermelho" />
                    <Card icon={ShieldQuestion} titulo="Dados insuficientes" valor={indicadores.dados_insuficientes} />
                </section>

                <section className="overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-sm">
                    <div className="border-b border-amber-100 p-5"><h2 className="font-semibold text-amber-950">Acompanhamento</h2><p className="mt-1 text-sm text-gray-500">Consulte o resumo e abra os detalhes para compreender os indicadores.</p></div>
                    <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-amber-50/70 text-xs uppercase text-amber-900/60"><tr><th className="px-5 py-3">Voluntário</th><th className="px-4 py-3">Eventos</th><th className="px-4 py-3">Situação</th><th className="px-5 py-3 text-right">Ações</th></tr></thead><tbody className="divide-y divide-amber-50">
                        {participantes.data.map((participante) => {
                            const totalEventos = participante.visitas_validas + participante.reunioes.presencas + participante.oficinas.presencas;
                            return <tr key={participante.id} className="hover:bg-amber-50/40"><td className="px-5 py-4"><span className="font-semibold text-amber-950">{participante.nome}</span><span className="block text-xs text-gray-500"><MapPin className="mr-1 inline size-3" />{participante.cidade} · {participante.cargos.join(', ')}</span></td><td className="px-4 py-4"><span className="font-semibold text-amber-950">{totalEventos}</span><span className="block text-xs text-gray-500">atividades documentadas</span></td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${estiloSituacao[participante.situacao]}`}>{textos[participante.situacao]}</span></td><td className="px-5 py-4 text-right"><button type="button" onClick={() => setParticipanteSelecionado(participante)} aria-label={`Ver detalhes de ${participante.nome}`} title={`Ver detalhes de ${participante.nome}`} className="inline-flex size-9 items-center justify-center rounded-full border border-amber-200 text-amber-700 transition hover:bg-amber-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500"><Eye className="size-4" aria-hidden /></button></td></tr>;
                        })}
                    </tbody></table></div>
                    {participantes.data.length === 0 && <p className="p-10 text-center text-sm text-gray-500">Nenhum voluntário encontrado para os filtros selecionados.</p>}
                    <div className="flex items-center justify-between border-t border-amber-100 p-4 text-sm text-gray-500"><span>Página {participantes.current_page} de {participantes.last_page}</span><div className="flex gap-2">{participantes.prev_page_url && <Link preserveScroll href={participantes.prev_page_url} className="rounded-full border border-amber-200 px-3 py-1.5 text-amber-800">Anterior</Link>}{participantes.next_page_url && <Link preserveScroll href={participantes.next_page_url} className="rounded-full border border-amber-200 px-3 py-1.5 text-amber-800">Próxima</Link>}</div></div>
                </section>

                <p className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-xs leading-relaxed text-gray-600"><strong>Leitura dos dados:</strong> as cores facilitam a consulta e não representam decisões automáticas. A coordenação deve considerar contexto, justificativas e histórico antes de qualquer decisão.</p>
            </div>

            <DetalhesModal participante={participanteSelecionado} filtros={filtros} onOpenChange={(aberto) => !aberto && setParticipanteSelecionado(null)} />
        </PainelLayout>
    );
}
...
function Card({ icon: Icone, titulo, valor, tom }: { icon: typeof UsersRound; titulo: string; valor: number; tom?: string }) {
    return <article className="rounded-xl border border-amber-100 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><div><p className="text-xs font-medium text-amber-900/60">{titulo}</p><p className="mt-1 text-2xl font-bold text-amber-950">{valor}</p></div><span className="flex size-9 items-center justify-center rounded-full bg-amber-50"><Icone className={`size-4 ${tom === 'verde' ? 'text-emerald-600' : tom === 'vermelho' ? 'text-red-500' : 'text-amber-600'}`} /></span></div></article>;
}
