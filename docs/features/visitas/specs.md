...
11. **Liberação de agenda (visitas hospitalares)**  
    ...
    O botão **Nova visita** só fica disponível quando uma cidade específica está selecionada e o respectivo mês está liberado. Quando bloqueado, a interface explica a indisponibilidade e a rota de criação repete a validação para impedir acesso direto ao formulário naquele contexto. Ao abrir o cadastro, o formulário preserva a cidade e o mês selecionados na agenda; no mês atual inicia no dia corrente e, em mês futuro, no primeiro dia. Meses bloqueados não são adicionados às opções de um novo cadastro, mas a edição preserva o mês original da visita. O salvamento de visitas hospitalares mantém a validação definitiva pela cidade do hospital e pela data informada.

    Filtros recebidos em `YYYY-MM` são interpretados sempre a partir do primeiro dia do mês. Isso evita que consultas realizadas nos dias 29, 30 ou 31 avancem indevidamente para o mês seguinte quando o mês selecionado possui menos dias.
