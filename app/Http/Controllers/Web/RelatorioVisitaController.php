...
{
    public function index(Request $request, Visita $visita): Response
    {
        $this->carregarVisita($visita);

        $relatorios = $visita->relatorios()
            ->with('autor:id,name')
            ->latest('enviado_em')
            ->get()
            ->each(fn (RelatorioVisita $relatorio) => $relatorio->setAttribute(
                'pode_editar',
                $this->podeEditar($request, $relatorio)
            ));
    ...
    public function show(Request $request, Visita $visita, RelatorioVisita $relatorio): Response
    {
